...
Eigen::Matrix4d NDT(PointCloudT::Ptr mapCloud, PointCloudT::Ptr source,Pose startingPose){
	...
	Eigen::Matrix4f initialTransform=transform3D(startingPose.rotation.yaw,startingPose.rotation.pitch,startingPose.rotation.roll,startingPose.position.x,startingPose.position.y,startingPose.position.z).cast<float>();
	pcl::console::TicToc time;
	time.tic();
	int iterations=60;
	...
	ndt.align(*cloud_ndt,initialTransform);
	Eigen::Matrix4d transformMatrix=ndt.getFinalTransformation().cast<double>();
	return transformMatrix;
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
			...
			//pose = ....
			// Eigen::Matrix4d poseTransform=ICP(mapCloud, filteredPCL, pose);
			Eigen::Matrix4d poseTransform=NDT(mapCloud,filteredPCL,pose);
			...
			renderPointCloud(viewer,correctPCL,"scan",Color(1,0,0));
